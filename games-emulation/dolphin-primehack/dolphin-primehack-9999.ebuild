...
if [[ ${PV} == *9999 ]]; then
	inherit git-r3
	EGIT_REPO_URI="https://github.com/shiiion/dolphin.git"
#	EGIT_SUBMODULES=(
#		Externals/mGBA/mgba
...
#		mv -vT "${WORKDIR}/minizip-ng-${MINIZIP-NG_COMMIT}" Externals/minizip-ng || die
#               mv -vT "${WORKDIR}/zlib-ng-${ZLIB-NG_COMMIT}" Externals/zlib-ng || die
#               mv -vT "${WORKDIR}/glslang-${GLSLANG_COMMIT}" Externals/glslang || die
#		die
		if use mgba; then
	...
	#ls -lah "${WORKDIR}"
        #mv -vT "${WORKDIR}/minizip-ng" Externals/minizip-ng || die
        #mv -vT "${WORKDIR}/zlib-ng" Externals/zlib-ng || die
        #mv -vT "${WORKDIR}/glslang" Externals/glslang || die
        #die
...
src_install() {
	cmake_src_install
	# Hack! Move any files named "dolphin-emu" and rename them to prevent collisions with vanilla dolphin
	mv -v "${D}/usr/bin/dsptool" "${D}/usr/bin/dsptool-primehack"
	mv -v "${D}/usr/bin/dolphin-tool" "${D}/usr/bin/primehack-tool"
	mv -v "${D}/usr/bin/dolphin-emu" "${D}/usr/bin/primehack"
	#find "${D}" -type f -name "*dolphin-emu*" -exec bash -c 'mv -v "$1" "${1/dolphin-emu/primehack}"' -- {} +\

	rm -rfv "${D}/usr/share/locale"
